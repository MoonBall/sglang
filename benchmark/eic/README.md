...
`--num-prompts` 表示最终请求数。
`--conversation-round` 表示共享同一个上下文的请求个数。
每个请求 prompt 格式大概为 `${shared_content}\n${question}`。

```bash
python3 bench_serving.py \
  --model=DeepSeek-R1-Distill-Llama-70B \
  --tokenizer=/sgl-workspace/models/DeepSeek-R1-Distill-Llama-70B/ \
  --base-url=http://127.0.0.1:8000 \
  --backend=vllm \
  --dataset-path=/sgl-workspace/models/LooGLE/data/summarization.jsonl  \
  --dataset-name=eic \
  --num-prompts=4 \
  ...
  --max-concurrency=16 \
  --fixed-output-len=300
```

### Multiturn benchmark

`--num-prompts` 表示最终请求数。
`--conversation-round` 表示一个多轮会话的轮数，也就是一个多轮会话相关的请求数。
