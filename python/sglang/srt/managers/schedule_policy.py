...
class PrefillAdder:
    ...
    def add_one_req(
        self, req: Req, has_chunked_req: bool, enable_hierarchical_cache: bool = False, enable_eic_cache: bool = False
    ):
        ...
        with self._lock_node(req.last_node):
            ...
            if (
                ...
                if enable_eic_cache and self.tree_cache.tp_size > 1:
                    prefix_len_tensor = torch.tensor([len(req.prefix_indices)], device="cpu")
                    torch.distributed.all_reduce(prefix_len_tensor, op=torch.distributed.ReduceOp.SUM, group=self.tree_cache.tp_group)
                    if prefix_len_tensor.item() != len(req.prefix_indices) * self.tree_cache.tp_size:
                        logger.error("prefix len is not consistent")
                logger.debug(f"req {req.rid} init load back, last node:{req.last_node.id}, prefix len:{len(req.prefix_indices)}")
                if enable_eic_cache:
                    ...
                    if self.tree_cache.tp_size > 1:
                        ...
                        torch.distributed.all_reduce(load_flag_tensor, op=torch.distributed.ReduceOp.SUM, group=self.tree_cache.tp_group)
                    ...
                    if not load_sucess:
                        ...
                        logger.error(f"req {req.rid} after load back failed, last node:{last_gpu_node.id}")
                    loading_check_end_ts = time.perf_counter()
                    logger.debug(f"batch_prefill loading check time {loading_check_end_ts - loading_check_start_ts}")
                logger.debug(f"after eic sync, req {req.rid} last node:{req.last_node.id}, prefix len:{len(req.prefix_indices)}")
